...
export default function TopBar({ running, onToggle, elapsed, entityCount, activeCount, threatCount }: Props) {
  return (
    <div style={S.bar}>
      <div style={S.brand}>
        <span style={S.logo}>◈ TRAXIS</span>
        <span style={S.opname}>Operation Blackedge</span>
      </div>
      <div style={S.stats}>
        ...
        <span style={S.statLabel}>T+<span style={S.statVal}>{fmt(elapsed)}</span></span>
      </div>
      <button
        onClick={onToggle}
